...
static bool i2c_write(const uint8_t *buffer, size_t len, bool stop = true, const uint8_t *prefix_buffer = nullptr, size_t prefix_len = 0);
...
float BNO08x::getPitch() {
    ...
    // Transform quaternion for new axes:
    // Original: Z-up, X-forward, Y-left
    // New:      Z-forward, X-left, Y-up
    // This requires swapping axes and adjusting signs
    float new_w = dqw;
    float new_x = dqy;  // Original Y becomes new X (with sign adjustment)
    float new_y = dqz;  // Original Z becomes new Y
    float new_z = dqx;  // Original X becomes new Z (with sign adjustment)

    // Normalize the transformed quaternion
    float norm = std::sqrt(new_w * new_w + new_x * new_x + new_y * new_y + new_z * new_z);
    new_w /= norm;
    new_x /= norm;
    new_y /= norm;
    new_z /= norm;

    // Roll (x-axis rotation) - now around the left-pointing axis
    float t0 = +2.0 * (new_w * new_x + new_y * new_z);
    float t1 = +1.0 - 2.0 * (new_x * new_x + new_y * new_y);
...
float BNO08x::getYaw() {
    ...
    // Transform quaternion for new axes
    float new_w = dqw;
    float new_x = dqy;  // Original Y becomes new X
    float new_y = dqz;  // Original Z becomes new Y
    float new_z = dqx;  // Original X becomes new Z
    ...
    // Pitch (y-axis rotation) - now around the up-pointing axis
    float t2 = +2.0 * (new_w * new_y - new_z * new_x);
    t2 = t2 > 1.0 ? 1.0 : t2;
    t2 = t2 < -1.0 ? -1.0 : t2;
    float yaw = -asin(t2);
...
float BNO08x::getRoll() {
    ...
    // Yaw (z-axis rotation) - now around the forward-pointing axis
    float t3 = +2.0 * (new_w * new_z + new_x * new_y);
    float t4 = +1.0 - 2.0 * (new_y * new_y + new_z * new_z);
 ...
 */
bool i2c_write(const uint8_t *buffer, size_t len, bool stop, const uint8_t *prefix_buffer, size_t prefix_len) {
...
bool i2c_read(uint8_t *buffer, size_t len, bool stop) {
    ...
    while (pos < len) {
        ...
        if (!_i2c_read(buffer + pos, read_len, read_stop)) return false;
        pos += read_len;
    }
    return true;
}
